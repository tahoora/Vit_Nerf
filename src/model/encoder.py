...
class ViTEncoder(nn.Module):
    ...
    def __init__(
        ...
        print("Using huggingface", backbone, "encoder")
        #self.feature_extracter = ViTFeatureExtractor.from_pretrained(backbone)
        self.model = ViTModel.from_pretrained(backbone, add_pooling_layer=False, output_hidden_states=True)


        # Following 2 lines need to be uncommented for older configs
        #self.model.fc = nn.Sequential()
        #self.model.avgpool = nn.Sequential()
        self.latent_size = 2561
    ...
    def forward(self, x):
        ...
        y = self.model(x)
        latents = []
        for z in y.hidden_states:
            #print('Before', z.shape)
            latents.append(torch.reshape(z, (batch_size, 197, 32, 24) ))
            #print('After', z.shape)
        ...
        #self.latent = self.model(x).hidden_states
        ...
        self.latent_size = self.latent.shape[1]
        return self.latent
...
class SpatialEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.use_custom_resnet:
            ...
            print('latent size for custom model', self.latent.shape)
        else:
